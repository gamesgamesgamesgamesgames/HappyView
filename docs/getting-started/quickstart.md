...
| Option | Best for |
|--------|----------|
| [**Railway**](deployment/railway) | Fastest path — one-click deploy of the full stack (HappyView + AIP + Tap + Postgres) |
| [**Docker Compose**](deployment/docker) | Local development with the full stack running in containers |
...
Open your HappyView instance in a browser. The built-in [dashboard](dashboard) is served at the root URL.
...
You can also upload lexicons manually via the dashboard or the [admin API](../reference/admin-api). See [Lexicons](../guides/lexicons) for the full details.
...
For custom query logic, attach a [Lua script](../guides/scripting).

## Next steps

You now have a working AppView. Here's where to go from here:

- [**Statusphere tutorial**](../tutorials/statusphere): end-to-end walkthrough building a complete AppView with record, query, and procedure lexicons
- [**Lexicons guide**](../guides/lexicons): target collections, backfill flag, network lexicons
- [**Lua Scripting**](../guides/scripting): custom query and procedure logic
