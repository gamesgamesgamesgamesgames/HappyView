...
pub async fn get_sync_state(
    ...
    space_id: &str,
    member_did: &str,
) -> Result<Option<SpaceSyncState>, AppError> {
    let sql = adapt_sql(
        "SELECT id, space_id, member_did, cursor, last_synced_at, status, error FROM space_sync_state WHERE space_id = ? AND member_did = ?",
        backend,
    );

    let row: Option<SyncStateRow> = sqlx::query_as(&sql)
        .bind(space_id)
        .bind(member_did)
        .fetch_optional(pool)
        .await
        .map_err(|e| AppError::Internal(format!("failed to get sync state: {e}")))?;

    row.map(parse_sync_state_row).transpose()
}
...
pub async fn upsert_sync_state(
    ...
    let sql = match backend {
        DatabaseBackend::Sqlite => {
            "INSERT OR REPLACE INTO space_sync_state (id, space_id, member_did, cursor, last_synced_at, status, error) VALUES (?, ?, ?, ?, ?, ?, ?)".to_string()
        }
        DatabaseBackend::Postgres => adapt_sql(
            "INSERT INTO space_sync_state (id, space_id, member_did, cursor, last_synced_at, status, error) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT (space_id, member_did) DO UPDATE SET cursor = EXCLUDED.cursor, last_synced_at = EXCLUDED.last_synced_at, status = EXCLUDED.status, error = EXCLUDED.error",
            backend,
        ),
    };
    ...
    sqlx::query(&sql)
        ...
        .execute(pool)
        .await
        .map_err(|e| AppError::Internal(format!("failed to upsert sync state: {e}")))?;

    Ok(())
}

pub async fn list_sync_states_for_space(
    pool: &sqlx::AnyPool,
    backend: DatabaseBackend,
    space_id: &str,
) -> Result<Vec<SpaceSyncState>, AppError> {
    let sql = adapt_sql(
        "SELECT id, space_id, member_did, cursor, last_synced_at, status, error FROM space_sync_state WHERE space_id = ? ORDER BY member_did ASC",
        backend,
    );

    let rows: Vec<SyncStateRow> = sqlx::query_as(&sql)
        .bind(space_id)
        .fetch_all(pool)
        .await
        .map_err(|e| AppError::Internal(format!("failed to list sync states: {e}")))?;

    rows.into_iter().map(parse_sync_state_row).collect()
}
...
pub async fn list_pending_syncs(
    ...
    let sql = adapt_sql(
        "SELECT id, space_id, member_did, cursor, last_synced_at, status, error FROM space_sync_state WHERE status = 'pending' OR status = 'error' ORDER BY last_synced_at ASC NULLS FIRST LIMIT 50",
        backend,
    );
    ...
    let rows: Vec<SyncStateRow> = sqlx::query_as(&sql)
        .fetch_all(pool)
        .await
        .map_err(|e| AppError::Internal(format!("failed to list pending syncs: {e}")))?;
...
fn parse_sync_state_row(r: SyncStateRow) -> Result<SpaceSyncState, AppError> {
    let status = SyncStatus::parse(&r.5)
        .ok_or_else(|| AppError::Internal(format!("invalid sync status: {}", r.5)))?;
