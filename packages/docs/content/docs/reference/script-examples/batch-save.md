...
1. Iterate over `input.items` and create a [`Record`](../../api-reference/lua/record-api.md) instance for each item.
2. Call [`Record.save_all()`](../../api-reference/lua/record-api.md#static-methods) to save all records in parallel, rather than one at a time.
